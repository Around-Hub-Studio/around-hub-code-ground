...
public class OptionalProcessingSample {
  ...
  public OptionalProcessingSample() {
    // filter
    Optional<SampleObject> sampleObject1 =
        Optional.of(getObject()).filter(object -> object.getName().equals("flature"));
    System.out.println(sampleObject1.get());

    Optional<SampleObject> sampleObject2 =
        Optional.of(getObject()).filter(object -> object.getName().equals("flature123"));
    System.out.println(sampleObject2.isPresent());

    // map
    Optional<String> StringObject1 =
        Optional.of(getObject()).map(sampleObject -> sampleObject.getName());
